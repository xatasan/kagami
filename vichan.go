...
func vichan_genthread(host string) func(b string, tc chan<- struct{ n, l float64 }) error {
	return func(b string, tc chan<- struct{ n, l float64 }) error {
		req := fmt.Sprintf("https://%s/%s/threads.json", host, b)
...
func newVichan(host string) Engine {
	return Engine{
		name: host,
		host: host,
		getFile: func(f string) string {
			return "http://" + host + "/src/" + f
		},
		getTmb: func(f string) string {
			switch path.Ext(f) {
			case ".jpeg", ".jpg", ".png":
				return "http://" + host + "/src/" + f + "s.jpg"
			default:
				return "https://" + host + "/static/file.jpg"
			}

		},
		getStatic: func(_, f string) string {
			return fmt.Sprintf("https://%s/static/%s", host, f)
		},
		genThread: func(board, no string) (Thread, error) {
			...
			// request /res/%d.json
			req := fmt.Sprintf("https://%s/%s/res/%s.json", host, board, no)
			resp, err := http.Get(req)
			if err != nil {
				return nil, err
			}
		...
		getThreads: vichan_genthread(host),
	}
}
