...
func getFile(file File) error {
	...
	remote := engine.getFile(file.Filename)
	return dl(local, remote)
}

func getThumbnail(file File) error {
	local := fmt.Sprintf("%s/%s", T_dir, file.Thumbnail)
	remote := engine.getTmb(file.Thumbnail)
	if remote == "" {
		return nil
	}
	return dl(local, remote)
...
func FDLqueue(dl <-chan File, wg *sync.WaitGroup) {
	defer wg.Done()
	for file := range dl {
